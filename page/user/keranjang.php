...
$query = $conn->prepare("SELECT id_keranjang, deskripsi, harga, ukuran, qty, kurir, total
                        FROM tbl_keranjang
                        JOIN tbl_barang ON tbl_keranjang.id_barang = tbl_barang.id_barang
                        WHERE tbl_keranjang.id_user = :id
                        GROUP BY tbl_keranjang.id_keranjang");
  ...
  <table class="news">
    ...
    foreach ($data as $value): ?>
      <tr>
        ...
        <td>
          <a class="tombol-merah" href="?page=keranjang_hapus&id=<?php echo $value['id_keranjang']; ?>">hapus</a>
        </td>
      </tr>
    <?php
    $no++;
